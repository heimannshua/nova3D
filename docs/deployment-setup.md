...
Google sign-in is credential-gated. After creating the OAuth web client, paste its ID and secret into Supabase Auth → Providers → Google, add the Supabase callback URL `https://<project-ref>.supabase.co/auth/v1/callback` in Google, and enable Google in Supabase Auth. Add each application callback URL to Supabase Auth → URL Configuration. `AUTH_ALLOWED_EMAILS` records the initial invite boundary for `heimannshua@gmail.com` and `daniel@orvex.ai`; it does not create users by itself. User creation and provider activation require an active Supabase project and its server credentials.

The app has no temporary password fallback. A deployment without valid Google/Supabase configuration remains private and returns users to the login page.
