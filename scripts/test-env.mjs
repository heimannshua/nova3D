...
const base = { NEXT_PUBLIC_SUPABASE_URL: 'https://example.supabase.co', NEXT_PUBLIC_SUPABASE_ANON_KEY: 'anon', SUPABASE_SERVICE_ROLE_KEY: 'service', AUTH_ALLOWED_EMAILS: 'heimannshua@gmail.com,daniel@orvex.ai', PAID_ADAPTERS_ENABLED: 'false', SYNTHETIC_DATA_ENABLED: 'true' };
...
const missingAllowlist = run({ ...base, APP_ENV: 'local', NEXT_PUBLIC_APP_ENV: 'local', AUTH_ALLOWED_EMAILS: '' });
if (missingAllowlist.status === 0 || !missingAllowlist.stderr.includes('AUTH_ALLOWED_EMAILS must contain at least one email address')) throw new Error('missing auth allowlist was not rejected');
